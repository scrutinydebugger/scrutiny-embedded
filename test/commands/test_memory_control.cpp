...
*/
TEST_F(TestMemoryControl, TestReadSingleRPV)
{
    uint8_t tx_buffer[32];
    scrutiny::Config new_config;
...
TEST_F(TestMemoryControl, TestReadMultipleRPV)
{
    uint8_t tx_buffer[32];
    scrutiny::Config new_config;
...
TEST_F(TestMemoryControl, TestReadMultipleRPVEachType)
{
    ...
    scrutiny_handler.init(&new_config);
    scrutiny_handler.comm()->connect();
    const unsigned int nb_vals = expected_encoding.size();
    // Make request
    unsigned int request_buffer_size = 8 + nb_vals * 2;
    ...
    for (auto p = expected_encoding.begin(); p != expected_encoding.end(); ++p)
    {
        ...
        response_payload_size += 2+p->second.size();
    }
...
TEST_F(TestMemoryControl, TestReadRPVBadRequest)
{
    const scrutiny::protocol::CommandId cmd = scrutiny::protocol::CommandId::MemoryControl;
...
TEST_F(TestMemoryControl, TestReadRPVNonExistingID)
{
    const scrutiny::protocol::CommandId cmd = scrutiny::protocol::CommandId::MemoryControl;
    const uint8_t subfn = static_cast<uint8_t>(scrutiny::protocol::MemoryControl::Subfunction::ReadRPV);
...
TEST_F(TestMemoryControl, TestReadRPVResponseOverflow)
{
    const scrutiny::protocol::CommandId cmd = scrutiny::protocol::CommandId::MemoryControl;
    const uint8_t subfn = static_cast<uint8_t>(scrutiny::protocol::MemoryControl::Subfunction::ReadRPV);
    const scrutiny::protocol::ResponseCode overflow = scrutiny::protocol::ResponseCode::Overflow;
