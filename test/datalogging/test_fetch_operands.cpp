//    test_fetch_operands.cpp
//        Test the capacity to decode an operand for log trigger
//
//   - License : MIT - See LICENSE file
//   - Project : Scrutiny Debugger (github.com/scrutinydebugger/scrutiny-embedded)
//
//    Copyright (c) 2021 Scrutiny Debugger

#include "scrutinytest/scrutinytest.hpp"
#include <cstring>

#include "scrutiny.hpp"
#include "scrutiny_test.hpp"

static bool rpv_read_callback(scrutiny::RuntimePublishedValue rpv, scrutiny::AnyType *outval, scrutiny::LoopHandler *const caller)
{
    static_cast<void>(caller);
    if (rpv.id == 0x1234 && rpv.type == scrutiny::VariableType::uint32)
    {
        outval->uint32 = 0xaabbccdd;
    }
    else if (rpv.id == 0x5678 && rpv.type == scrutiny::VariableType::float32)
    {
        outval->float32 = 3.1415926f;
    }
    else
    {
        return false;
    }

    return true;
}

class TestFetchOperands : public ScrutinyTest
{
  protected:
    scrutiny::Timebase tb;
    scrutiny::MainHandler scrutiny_handler;
    scrutiny::Config config;

    unsigned char _rx_buffer[128];
    unsigned char _tx_buffer[128];

#if SCRUTINY_SUPPORT_PROTECTED_REGIONS
    unsigned char forbidden_buffer[128];
    unsigned char forbidden_buffer2[128];
    unsigned char readonly_buffer[128];
    unsigned char readonly_buffer2[128];

    scrutiny::AddressRange readonly_ranges[2];
    scrutiny::AddressRange forbidden_ranges[2];
#endif
    scrutiny::RuntimePublishedValue rpvs[2];

    TestFetchOperands() :
        ScrutinyTest(),
        tb(),
        scrutiny_handler(),
        config(),
        _rx_buffer(),
        _tx_buffer()
#if SCRUTINY_SUPPORT_PROTECTED_REGIONS
        ,
        forbidden_buffer(),
        forbidden_buffer2(),
        readonly_buffer(),
        readonly_buffer2()
#endif
    {
#if SCRUTINY_SUPPORT_PROTECTED_REGIONS
        readonly_ranges[0] = scrutiny::tools::make_address_range(readonly_buffer, sizeof(readonly_buffer)),
        readonly_ranges[1] = scrutiny::tools::make_address_range(readonly_buffer2, sizeof(readonly_buffer2));

        forbidden_ranges[0] = scrutiny::tools::make_address_range(forbidden_buffer, sizeof(forbidden_buffer));
        forbidden_ranges[1] = scrutiny::tools::make_address_range(forbidden_buffer, sizeof(forbidden_buffer2));
#endif
        rpvs[0].id = 0x1234;
        rpvs[0].type = scrutiny::VariableType::uint32;
        rpvs[1].id = 0x5678;
        rpvs[1].type = scrutiny::VariableType::float32;
    }

    virtual void SetUp()
    {
        config.set_buffers(_rx_buffer, sizeof(_rx_buffer), _tx_buffer, sizeof(_tx_buffer));
#if SCRUTINY_SUPPORT_PROTECTED_REGIONS
        config.set_readonly_address_range(readonly_ranges, sizeof(readonly_ranges) / sizeof(readonly_ranges[0]));
        config.set_forbidden_address_range(forbidden_ranges, sizeof(forbidden_ranges) / sizeof(forbidden_ranges[0]));
#endif
        config.set_published_values(rpvs, sizeof(rpvs) / sizeof(rpvs[0]), rpv_read_callback);

        scrutiny_handler.init(&config);
    }
};

using namespace scrutiny::datalogging;

TEST_F(TestFetchOperands, TestFetchLiteral)
{
    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;

    Operand operand;
    operand.common.type = OperandType::Literal;
    operand.literal.val = 0.1234f;
    bool success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);
    EXPECT_EQ(val_type_pair.valtype, scrutiny::VariableType::float32);
    EXPECT_EQ(val_type_pair.val.float32, 0.1234f);
}

TEST_F(TestFetchOperands, TestFetchVar)
{
    float my_var = 3.1415926f;

    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;

    Operand operand;
    operand.common.type = OperandType::Var;
    operand.var.addr = &my_var;
    operand.var.datatype = scrutiny::VariableType::float32;

    bool success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);

    EXPECT_EQ(val_type_pair.valtype, scrutiny::VariableType::float32);
    EXPECT_EQ(val_type_pair.val.float32, my_var);
}

TEST_F(TestFetchOperands, TestFetchRPV)
{
    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;

    Operand operand;
    operand.common.type = OperandType::Rpv;

    operand.rpv.id = 0x1234;
    bool success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);
    EXPECT_EQ(val_type_pair.valtype, scrutiny::VariableType::uint32);
    EXPECT_EQ(val_type_pair.val.uint32, 0xaabbccdd);

    operand.rpv.id = 0x5678;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);
    EXPECT_EQ(val_type_pair.valtype, scrutiny::VariableType::float32);
    EXPECT_EQ(val_type_pair.val.float32, 3.1415926f);
}

TEST_F(TestFetchOperands, TestFetchVarBit)
{
#pragma pack(push, 1)
    struct
    {
        int16_t : 3;
        int16_t val : 11;
        int16_t : 2;
    } my_struct;
#pragma pack(pop)

    my_struct.val = -1000;

    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;

    Operand operand;
    operand.common.type = OperandType::VarBit;

    operand.varbit.addr = &my_struct;
    operand.varbit.datatype = scrutiny::VariableType::sint16;
    operand.varbit.bitoffset = 3;
    operand.varbit.bitsize = 11;

    bool success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);
    EXPECT_EQ(val_type_pair.valtype, scrutiny::VariableType::sint16);
    EXPECT_EQ(val_type_pair.val.sint16, my_struct.val);
}

TEST_F(TestFetchOperands, TestBadOperandType)
{
    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;

    Operand operand;
    operand.common.type = static_cast<OperandType::eOperandType>(0xff);

    bool success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_FALSE(success);
}

TEST_F(TestFetchOperands, TestFetchInexistandRPV)
{
    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;

    Operand operand;
    operand.common.type = OperandType::Rpv;
    operand.rpv.id = 0xAAAA;

    bool success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_FALSE(success);
}

TEST_F(TestFetchOperands, TestFetchBitfieldsLimits)
{
#pragma pack(push, 1)
    struct
    {
        int32_t a;
        int32_t b;
        int32_t c;
        int32_t d;
    } my_struct;
#pragma pack(pop)

    my_struct.a = 0;
    my_struct.b = 0;
    my_struct.c = 0;
    my_struct.d = 0;

    scrutiny::AnyValAndTypePair val_type_pair;
    val_type_pair.valtype = scrutiny::VariableType::unknown;
    bool success = false;

    Operand operand;
    operand.common.type = OperandType::VarBit;
    operand.varbit.addr = &my_struct;
    operand.varbit.datatype = scrutiny::VariableType::sint16;

    operand.varbit.bitoffset = 31;
    operand.varbit.bitsize = 1;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);

    operand.varbit.bitoffset = 32;
    operand.varbit.bitsize = 1;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);

#if SCRUTINY_SUPPORT_64BITS
    EXPECT_TRUE(success);
#else
    EXPECT_FALSE(success);
#endif

    operand.varbit.bitoffset = 0;
    operand.varbit.bitsize = 32;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_TRUE(success);

    operand.varbit.bitoffset = 0;
    operand.varbit.bitsize = 33;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
#if SCRUTINY_SUPPORT_64BITS
    EXPECT_TRUE(success);
#else
    EXPECT_FALSE(success);
#endif

    operand.varbit.bitoffset = 63;
    operand.varbit.bitsize = 1;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
#if SCRUTINY_SUPPORT_64BITS
    EXPECT_TRUE(success);
#else
    EXPECT_FALSE(success);
#endif

    operand.varbit.bitoffset = 64;
    operand.varbit.bitsize = 1;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_FALSE(success);

    operand.varbit.bitoffset = 0;
    operand.varbit.bitsize = 64;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
#if SCRUTINY_SUPPORT_64BITS
    EXPECT_TRUE(success);
#else
    EXPECT_FALSE(success);
#endif

    operand.varbit.bitoffset = 0;
    operand.varbit.bitsize = 65;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_FALSE(success);

    operand.varbit.bitoffset = 0;
    operand.varbit.bitsize = 0;
    success = fetch_operand(&scrutiny_handler, &operand, &val_type_pair, SCRUTINY_NULL);
    EXPECT_FALSE(success);
}
