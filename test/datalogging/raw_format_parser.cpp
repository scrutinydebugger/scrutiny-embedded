...
void RawFormatParser::parse(uint32_t entry_count)
{
    ...
    for (uint16_t i = 0; i < m_config->items_count; i++)
    {
        ...
        {
            elem_size = scrutiny::tools::tools::get_type_size_8bits(m_main_handler->get_rpv_type(m_config->items_to_log[i].data.rpv.id));
        }
    ...
    for (uint32_t i = 0; i < entry_count; i++)
    {
        ...
        for (uint16_t j = 0; j < m_config->items_count; j++)
        {
            ...
            {
                elem_size = scrutiny::tools::tools::get_type_size_8bits(m_main_handler->get_rpv_type(m_config->items_to_log[j].data.rpv.id));
            }
