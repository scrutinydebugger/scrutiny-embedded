...
#error
        ...
        {
            SCRUTINY_STATIC_ASSERT(sizeof(uint32_t) == sizeof(float) * (CHAR_BIT / 8), "Expect float to be 32 bits");
            uint32_t uv;
...
#elif CHAR_BIT == 16
            SCRUTINY_STATIC_ASSERT(sizeof(float) == 2, "Expect float to be 2 char wide (32 bits)");
