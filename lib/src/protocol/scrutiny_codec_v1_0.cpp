...
namespace scrutiny
{
    namespace protocol
    {
        ...
        {
            constexpr uint16_t datalen = sizeof(response_data->buffer_size) + sizeof(response_data->data_encoding) + sizeof(response_data->max_signal);
            if (datalen > MINIMUM_TX_BUFFER_SIZE && datalen > response->data_max_length)
            {
                return ResponseCode::Overflow;
            }
            ...
            cursor += codecs::encode_8_bits(response_data->max_signal, &response->data[cursor]);
            response->data_length = cursor;
